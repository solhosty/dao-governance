...
export default function DaoDetailPage({ params }: DAOPageProps) {
  ...
  const { data: state } = useReadContract({
    ...
    query: { enabled: Boolean(daoAddress) },
  });

  const { writeContract, isPending } = useWriteContract();

  if (!daoAddress) {
    return <p>DAO not found. Provide a DAO address or numeric ID.</p>;
  }
    ...
    <main className="space-y-4">
      ...
      <section className="rounded-lg border border-white/40 bg-white/60 p-4 shadow-glass backdrop-blur-md">
        ...
        <div className="grid gap-3">
          ...
          />
          <p className="text-xs text-slate-500">Current proposal state: {String(state ?? "unknown")}</p>
          <button
